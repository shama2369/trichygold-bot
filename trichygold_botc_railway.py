...
async def tasks_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for task in tasks:
            ...
            time_minutes = task.get("reminder_interval")
            ...
            message += f"• Description: {task_desc}\n"
            message += f"• Priority: {priority}\n"
            if time_minutes:
                message += f"• Time: {time_minutes} minutes\n"
            if due_date:
                message += f"• Due: {due_date}\n"
            if is_admin:
                message += f"• Assigned to: {', '.join([db.employees.find_one({'chat_id': str(id)})['name'] for id in assigned_to])}\n"
